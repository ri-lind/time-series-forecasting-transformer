...
This section presents the forecasting performance of SAMFormer, TimesFM, Time-MoE, TimeGPT, and regression baselines across different datasets (weather, finance, energy, and healthcare) and forecasting horizons. We evaluated the models using the root mean squared error (RMSE), the mean absolute error (MAE), and the coefficient of determination (R\textsuperscript{2}) to assess accuracy and reliability. Detailed metric breakdowns, including MAPE, MASE, and explained variance, are provided in Appendix \ref{appendixA}. Moreover, Appendix \ref{appendixB} contains the predictions made by the models.
